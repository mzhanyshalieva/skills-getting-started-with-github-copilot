document.addEventListener("DOMContentLoaded", () => {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsList = (details.participants || []).map((p) => `
            <li class="participant-item">
              ...
              <button class="remove-btn" data-activity="${name}" data-email="${p}" aria-label="Remove participant">✕</button>
            </li>
        `).join("");
  ...
  activitiesList.addEventListener("click", async (event) => {
    const btn = event.target.closest && event.target.closest('.remove-btn');
    if (!btn) return;
